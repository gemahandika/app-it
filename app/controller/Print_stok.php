<?php
class Print_stok extends Controller
{
    ...
    public function tambah()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $data = [
                ...
                'status' => $_POST['status'],
                'keterangan' => 'Stok',
                'date_terima' =>  $_POST['date_terima'],
                'remaks' => $_POST['remaks']
            ];

            if ($this->model('Printer_models')->addPrinterStok($data) > 0) {
                Flasher::setFlash('berhasil', 'ditambahkan', 'success');
            } else {
                Flasher::setFlash('gagal', 'ditambahkan', 'danger');
            }

            header('Location: ' . BASE_URL . '/print_stok');
            exit;
        }
    }
